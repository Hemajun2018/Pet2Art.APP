...
export default function PetArtGenerator() {
  ...
  const [dialogSelectedTemplate, setDialogSelectedTemplate] = useState<PetTemplate | null>(null)
  const [dialogSelectedCategory, setDialogSelectedCategory] = useState<string>("all")
  const [dialogPetImage, setDialogPetImage] = useState<File | null>(null)
  const [dialogPetImagePreview, setDialogPetImagePreview] = useState<string>("")
  ...
  // 弹窗中的宠物图片处理函数
  const processDialogImageFile = (file: File) => {
    const validation = validateImageFile(file)
    if (!validation.isValid) {
      alert(validation.reason)
      return
    ...
    setDialogPetImage(file)
    const reader = new FileReader()
    reader.onload = (e) => {
      setDialogPetImagePreview(e.target?.result as string)
    }
    reader.readAsDataURL(file)
  }

  const handleDialogPetImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]
    if (file) {
      processDialogImageFile(file)
    }
  }

  const handleDialogDrop = (event: React.DragEvent<HTMLDivElement>) => {
    event.preventDefault()
    const file = event.dataTransfer.files[0]
    if (file) {
      processDialogImageFile(file)
          ...
          <Card className="lg:col-span-3">
            ...
            <CardContent className="space-y-4">
              ...
              <Button 
                ...
                disabled={(!petImage && !showReferenceImage) || (!petImage && !selectedTemplate) || isGenerating}
